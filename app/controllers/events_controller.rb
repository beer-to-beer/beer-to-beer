class EventsController < ApplicationController
  ...
  def index
    ...
    @events_club_inaktiv = Event.joins(:club).where('events.sdate > ?', Date.current).where('clubs.active = ?', false)

    @events_old = Event.joins(:club).where('events.sdate <=?', Date.current)
